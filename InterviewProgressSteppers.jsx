...
/* ──────────────────────────────────────────────────────────────────────────
   ...
   Six stepper / progress-tracker variants with a standardised app-layer
   adapter contract and free navigation.
   ────────────────────────────────────────────────────────────────────────── */
...
/* ────────────────────────────────────────────────────────────
   Stepper Adapter Contract
   ──────────────────────────────────────────────────────────────
   Pass these props to <StepperShell>. The shell auto-injects
   setCurrent (wrapped) and navigationMode into the variant
   via React.cloneElement — you don't pass them to the variant.

   Required
   ─────────
   steps[]       { label, description?, Icon? }
   current       Controlled index (0-based)
   setCurrent    State setter (step: number) => void
   total         Total step count

   Optional
   ─────────
   navigationMode  'free' (default) | 'linear'
     free   — all step indicators are clickable; future steps
              render as "upcoming" (accent-tinted, pointer cursor)
     linear — only completed + current are clickable; future
              steps are greyed out with cursor:not-allowed

   onNavigate    (newStep: number, prevStep: number) => void
     Fires on every navigation action — Back/Next button clicks
     and direct step indicator clicks — before the state update.
     Use this to sync the app layer:

     // Voices — scroll a continuous question list
     onNavigate={(next, prev) =>
       questionListRef.current?.scrollToItem(STEP_QUESTION_MAP[next])
     }

     // Analytics
     onNavigate={(next) => track('stepper_navigate', { step: next })}

     // Unlock content panel
     onNavigate={(next) => preloadPanel(next)}
   ──────────────────────────────────────────────────────────────── */
...
function StepperShell({
  ...
  const navigate = useCallback(
    (newStep) => {
      const clamped = Math.max(0, Math.min(total - 1, newStep));
      if (clamped === current) return;
      onNavigate?.(clamped, current);
      setCurrent(clamped);
    },
    [current, total, setCurrent, onNavigate],
  );
  ...
  // Inject navigate + navigationMode into the direct child variant.
  // Apps only need to pass these to StepperShell, not to the variant.
  const enhanced = Children.map(children, (child) =>
    isValidElement(child)
      ? cloneElement(child, { setCurrent: navigate, navigationMode })
      : child,
  );
  ...
  return (
    <section className="ips-card" aria-labelledby={`ttl-${title}`}>
      ...
      <div className="ips-card-body">{enhanced}</div>
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
function ClassicNumberedStepper({ steps, current, setCurrent, navigationMode = "free" }) {
  return (
    <ol className="v1-list" role="list" aria-label="Interview progress">
      {steps.map((step, i) => {
        const status =
          i < current ? "complete" :
          i === current ? "active" :
          navigationMode === "free" ? "upcoming" : "pending";
        const clickable = navigationMode === "free" || i <= current;
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
function MinimalDotStepper({ steps, current, setCurrent, navigationMode = "free" }) {
    ...
    <div className="v2-wrap">
        ...
        {steps.map((_, i) => {
          ...
          return (
            <span
              ...
              className={[
                "v2-dot",
                i === current                                    ? "v2-dot-current"  : "",
                i < current                                      ? "v2-dot-done"     : "",
                isFuture && navigationMode === "free"            ? "v2-dot-upcoming" : "",
              ].join(" ").trim()}
      ...
      <div className="v2-labels">
        {steps.map((step, i) => {
          ...
          const isUpcoming = i > current && navigationMode === "free";
          const isLocked   = i > current && navigationMode !== "free";
          return (
            <button
              type="button"
              key={i}
              className={[
                "v2-label",
                isCurrent  ? "v2-label-active"   : "",
                isUpcoming ? "v2-label-upcoming"  : "",
                isLocked   ? "v2-label-future"    : "",
              ].join(" ").trim()}
              style={{ left: `${(i / (total - 1)) * 100}%` }}
              onClick={() => clickable && setCurrent(i)}
              disabled={!clickable}
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
function SegmentedStepper({ steps, current, setCurrent, navigationMode = "free" }) {
  return (
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
function CircularRingStepper({ steps, current, setCurrent, navigationMode = "free" }) {
  const total  = steps.length;
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
function PillIconStepper({ steps, current, setCurrent, navigationMode = "free" }) {
  return (
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
function VerticalTimelineStepper({ steps, current, setCurrent, navigationMode = "free" }) {
  return (
...
/* ────────────────────────────────────────────────────────────
   Main demo: brand toggle + 6 stepper cards + nav toast
   ──────────────────────────────────────────────────────────── */
export default function App() {
  const [brand, setBrand] = useState("fho");
  ...
  // navLog drives the onNavigate toast — each new id forces a CSS animation reset
  const [navLog, setNavLog] = useState(null);

  // Shared adapter hook — in real apps this scrolls a question list,
  // loads a panel, fires analytics, etc.
  const handleNavigate = useCallback((next, prev) => {
    setNavLog({ next, prev, id: Date.now() });
  }, []);
    ...
    <>
      ...
      <div className="ips-root" data-brand={brand}>
        <header className="ips-page-head">
          <div>
            ...
            <p className="ips-page-sub">
              Six variants with free navigation — click any step to jump directly,
              or use Back / Next. Every navigation fires{" "}
              <code>onNavigate(next, prev)</code> so your app layer can sync
              (scroll a question list, load a panel, fire analytics).
            </p>
          </div>
          <div className="ips-toggle" role="radiogroup" aria-label="Brand theme">
            <button
        ...
        <div className="ips-grid">
          <StepperShell title="01 — Classic numbered" kicker="Variant 01"
            current={c1} setCurrent={s1} total={STEPS.length} onNavigate={handleNavigate}>
            <ClassicNumberedStepper steps={STEPS} current={c1} />
          </StepperShell>

          <StepperShell title="02 — Minimal dot + bar" kicker="Variant 02"
            current={c2} setCurrent={s2} total={STEPS.length} onNavigate={handleNavigate}>
            <MinimalDotStepper steps={STEPS} current={c2} />
          </StepperShell>

          <StepperShell title="03 — Segmented bar" kicker="Variant 03"
            current={c3} setCurrent={s3} total={STEPS.length} onNavigate={handleNavigate}>
            <SegmentedStepper steps={STEPS} current={c3} />
          </StepperShell>

          <StepperShell title="04 — Circular ring" kicker="Variant 04"
            current={c4} setCurrent={s4} total={STEPS.length} onNavigate={handleNavigate}>
            <CircularRingStepper steps={STEPS} current={c4} />
          </StepperShell>

          <StepperShell title="05 — Vertical timeline" kicker="Variant 05"
            current={c5} setCurrent={s5} total={STEPS.length} onNavigate={handleNavigate}>
            <VerticalTimelineStepper steps={STEPS} current={c5} />
          </StepperShell>

          <StepperShell title="06 — Pill icons" kicker="Variant 06"
            current={c6} setCurrent={s6} total={STEPS.length} onNavigate={handleNavigate}>
            <PillIconStepper steps={STEPS} current={c6} />
          </StepperShell>
        </div>
        ...
        <footer className="ips-page-foot">
          ...
          <p>
            Drop each variant into your codebase and theme via the CSS variables
            on <code>.ips-root[data-brand]</code>. Wire <code>onNavigate</code> to
            sync your app layer. Pass <code>navigationMode="linear"</code> to
            re-enable forward-lock for guided flows.
          </p>
        </footer>

        {/* onNavigate demo toast — shows the adapter firing on each navigation */}
        {navLog && (
          <div
            key={navLog.id}
            className="ips-toast"
            ...
            aria-atomic="true"
          >
            onNavigate({navLog.prev + 1} → {navLog.next + 1})
          </div>
        )}
      </div>
    </>
...
.ips-toggle-btn.is-on { background: var(--c-accent); color: var(--c-on-accent); }
...
.v1-item { display: flex; align-items: center; flex: 1; position: relative; min-width: 0; }
.v1-item:last-child { flex: 0 0 auto; }
.v1-step-btn {
  display: flex; flex-direction: column; align-items: center; gap: 8px;
  background: transparent; border: 0; padding: 4px; min-width: 80px;
}
.v1-circle {
  width: 36px; height: 36px; border-radius: 999px;
  display: inline-flex; align-items: center; justify-content: center;
  background: var(--c-surface); color: var(--c-text-mute);
  border: 2px solid var(--c-border); font-weight: 600; font-size: 14px;
...
.v1-label {
  ...
  text-align: center; transition: color 260ms ease; max-width: 100px;
}
.v1-line {
  flex: 1; height: 2px; background: var(--c-track);
  margin: 0 -2px; position: relative; top: -10px;
  border-radius: 999px; transition: background-color 360ms ease;
}
/* active */
.v1-active .v1-circle {
...
/* complete */
.v1-complete .v1-circle { background: var(--c-accent); border-color: var(--c-accent); color: var(--c-on-accent); }
.v1-complete .v1-label { color: var(--c-text); }
.v1-complete + .v1-item .v1-line,
.v1-complete .v1-line { background: var(--c-accent); }
/* upcoming (free-mode future) */
.v1-upcoming .v1-circle {
  border-color: color-mix(in srgb, var(--c-accent) 40%, var(--c-border));
  color: var(--c-text-mute);
}
.v1-upcoming .v1-label { color: var(--c-text-mute); }
/* pending (linear-mode future) — greyed, locked */
.v1-pending .v1-circle { opacity: 0.5; }
.v1-pending .v1-label  { opacity: 0.5; }
/* hover: applies to complete + upcoming (both enabled) */
.v1-step-btn:hover:not(:disabled) .v1-circle { border-color: var(--c-accent); }

/* ────────────────────────────────────────────────────────────
   Variant 2 — Minimal dot + bar
   ──────────────────────────────────────────────────────────── */
.v2-wrap { width: 100%; padding: 8px 12px 32px; position: relative; }
.v2-track {
  position: relative; height: 6px; background: var(--c-track);
  border-radius: 999px; margin: 22px 0 12px;
}
.v2-fill {
...
.v2-dot-current  {
  background: var(--c-accent); border-color: var(--c-accent); width: 14px; height: 14px;
  box-shadow: 0 0 0 5px color-mix(in srgb, var(--c-accent) 18%, transparent);
}
.v2-dot-upcoming { border-color: color-mix(in srgb, var(--c-accent) 40%, var(--c-border)); }
.v2-labels { position: relative; height: 36px; margin-top: 10px; }
.v2-label {
  position: absolute; top: 0; transform: translateX(-50%);
  background: transparent; border: 0; padding: 4px 6px;
  font-size: 12px; font-weight: 500; color: var(--c-text-mute);
  white-space: nowrap; transition: color 240ms ease, font-weight 240ms ease;
}
.v2-label-active   { color: var(--c-accent); font-weight: 700; }
.v2-label-upcoming { color: var(--c-text-mute); }
.v2-label-upcoming:hover:not(:disabled) { color: var(--c-accent); }
.v2-label-future   { opacity: 0.45; cursor: not-allowed; }
...
.v3-seg-bar {
  ...
  background: var(--c-track); transition: background 380ms ease, transform 260ms ease;
}
.v3-seg-meta { display: flex; flex-direction: column; gap: 2px; min-width: 0; }
.v3-seg-num  {
  font-size: 11px; font-weight: 700; letter-spacing: 0.08em;
  color: var(--c-text-mute); transition: color 240ms ease;
}
.v3-seg-label {
  font-size: 13px; font-weight: 600; color: var(--c-text);
  transition: color 240ms ease; overflow: hidden; text-overflow: ellipsis; white-space: nowrap;
}
/* complete */
...
/* upcoming — neutral body, accent only hints on hover (matches v1/v2/v5) */
.v3-upcoming .v3-seg-bar   { background: var(--c-track); }
.v3-upcoming .v3-seg-num   { color: var(--c-text-mute); }
.v3-upcoming .v3-seg-label { color: var(--c-text-mute); }
.v3-upcoming:hover:not(:disabled) .v3-seg-bar { background: color-mix(in srgb, var(--c-accent) 40%, var(--c-track)); }
/* pending (linear mode) */
.v3-pending .v3-seg-num,
.v3-pending .v3-seg-label { color: var(--c-text-mute); opacity: 0.5; }
/* general hover for complete */
...
/* ────────────────────────────────────────────────────────────
   ...
   ──────────────────────────────────────────────────────────── */
.v4-wrap { width: 100%; display: flex; flex-direction: column; gap: 18px; }
.v4-main { display: flex; align-items: center; gap: 18px; }
.v4-ring  { position: relative; flex-shrink: 0; display: inline-grid; place-items: center; }
.v4-ring svg { display: block; }
.v4-ring-progress { transition: stroke-dashoffset 520ms cubic-bezier(.2,.7,.2,1); }
.v4-ring-num {
  position: absolute; inset: 0; display: grid; place-items: center;
  font-family: var(--font-head); color: var(--c-text); pointer-events: none;
}
.v4-ring-current {
...
.v4-info-label {
  ...
  margin: 0 0 2px; color: var(--c-text); letter-spacing: -0.01em;
}
.v4-info-desc { font-size: 13px; color: var(--c-text-mute); margin: 0; }
.v4-ticks { display: grid; grid-template-columns: repeat(5, 1fr); gap: 6px; }
.v4-tick  {
  background: transparent; border: 0; padding: 0;
...
.v4-tick-bar {
  height: 4px; border-radius: 999px; background: var(--c-track);
  transition: background-color 320ms ease, transform 220ms ease;
}
.v4-tick-label {
  font-size: 11px; font-weight: 500; color: var(--c-text-mute);
...
/* upcoming — neutral body, accent only hints on hover (matches v1/v2/v5) */
.v4-tick-upcoming .v4-tick-bar   { background: var(--c-track); }
.v4-tick-upcoming .v4-tick-label { color: var(--c-text-mute); }
.v4-tick-upcoming:hover:not(:disabled) .v4-tick-bar { background: color-mix(in srgb, var(--c-accent) 40%, var(--c-track)); }
/* pending (linear mode) */
.v4-tick-pending .v4-tick-bar   { opacity: 0.4; }
.v4-tick-pending .v4-tick-label { opacity: 0.4; }
/* general hover */
...
.v6-icon {
  ...
  background: var(--c-bg); color: var(--c-text-mute); flex-shrink: 0;
  transition: background-color 220ms ease, color 220ms ease;
}
.v6-text { display: inline-flex; flex-direction: column; line-height: 1.1; text-align: left; }
.v6-num  { font-size: 10px; font-weight: 700; letter-spacing: 0.1em; color: var(--c-text-mute); margin-bottom: 2px; }
.v6-label { font-size: 13px; font-weight: 600; color: inherit; white-space: nowrap; }
/* active */
.v6-active {
...
.v6-upcoming .v6-icon {
  ...
  color: color-mix(in srgb, var(--c-accent) 55%, var(--c-text-mute));
}
.v6-upcoming:hover:not(:disabled) {
  border-color: color-mix(in srgb, var(--c-accent) 70%, var(--c-border));
  color: var(--c-text);
}
/* pending (linear mode) */
.v6-pending { opacity: 0.4; }

/* ────────────────────────────────────────────────────────────
   Variant 5 — Vertical timeline
   ──────────────────────────────────────────────────────────── */
.v5-list { list-style: none; margin: 0; padding: 0; width: 100%; display: flex; flex-direction: column; }
.v5-item { padding: 0; }
.v5-step-btn {
  width: 100%; background: transparent; border: 0; padding: 8px 0;
  display: grid; grid-template-columns: 36px 1fr; gap: 14px;
  text-align: left; border-radius: 8px; transition: background-color 180ms ease;
}
...
.v5-node {
  ...
  color: var(--c-text-mute); display: inline-flex; align-items: center; justify-content: center;
...
.v5-connector {
  flex: 1; width: 2px; background: var(--c-track);
  margin-top: 2px; min-height: 24px; transition: background-color 360ms ease;
}
.v5-content { padding: 2px 0 14px; }
.v5-label   { display: block; font-size: 14px; font-weight: 600; color: var(--c-text); transition: color 240ms ease; }
.v5-desc    { display: block; font-size: 12px; color: var(--c-text-mute); margin-top: 2px; }
/* active */
.v5-active .v5-node {
  background: var(--c-accent); border-color: var(--c-accent); color: var(--c-on-accent);
  transform: scale(1.06);
...
/* complete */
.v5-complete .v5-node      { background: var(--c-accent); border-color: var(--c-accent); color: var(--c-on-accent); }
.v5-complete .v5-connector { background: var(--c-accent); }
/* upcoming */
.v5-upcoming .v5-node  {
  border-color: color-mix(in srgb, var(--c-accent) 42%, var(--c-border));
  color: color-mix(in srgb, var(--c-accent) 50%, var(--c-text-mute));
}
.v5-upcoming .v5-label { color: var(--c-text-mute); }
.v5-upcoming .v5-step-btn:hover:not(:disabled) .v5-node {
  border-color: color-mix(in srgb, var(--c-accent) 70%, var(--c-border));
}
/* pending (linear mode) */
.v5-pending .v5-label { color: var(--c-text-mute); opacity: 0.5; }
.v5-pending .v5-node  { opacity: 0.45; }
...
.ips-accent-strip { height: 4px; border-radius: 999px; background: var(--gradient); }
.ips-page-foot p  { font-size: 12px; color: var(--c-text-mute); margin: 0; }
.ips-page-foot code {
...
/* ── Completion burst ───────────────────────────────────── */
.ips-done-wrap { position: relative; display: inline-flex; }
.ips-burst     { position: absolute; inset: 0; pointer-events: none; display: grid; place-items: center; }
.ips-spark {
...
.ips-btn-done-burst { animation: done-pop 0.5s cubic-bezier(.2,.8,.3,1); }
@keyframes done-pop {
  0%   { transform: scale(1); }
